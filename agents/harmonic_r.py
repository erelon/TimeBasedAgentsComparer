...
class HarmonicRLAgent2(RLAgent):
    ...
    def learn(self, state, action, reward, next_state, time):
        ...
        if not self.with_rho_trick or (self.with_rho_trick and action == best_current_action):
            self.reciprocal_rho = (1 - self.rho_learning_rate) * self.reciprocal_rho + self.rho_learning_rate * (recip_td_error)
...
class AdaptiveHarmonicRLAgent(RLAgent):
    ...
    def learn(self, state, action, reward, next_state, time):
        ...
        if not self.with_rho_trick or (self.with_rho_trick and action == best_current_action):
            self.reciprocal_rho = (1 - self.rho_learning_rate) * self.reciprocal_rho + self.rho_learning_rate * (time / (reward))
            self.rho = 1 / self.reciprocal_rho
            self.total_time += time
            self.total_reward += reward
